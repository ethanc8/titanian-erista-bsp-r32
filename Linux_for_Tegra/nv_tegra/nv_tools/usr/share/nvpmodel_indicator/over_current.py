...
class SocthermIRQ(object):
    def __init__(self, path):
        self.path = path
        self.irq_cnt_prev = self.irq_cnt()

    def get_new_event(self):
        new = self.irq_cnt() - self.irq_cnt_prev
        self.irq_cnt_prev += new
        return new
...
def oc_event_t186():
    hwmons = [p for p in Path('/sys/class/hwmon').glob('*')]
    for hwmon in hwmons:
        name = hwmon.joinpath('name').read_text().strip()
        if name == 'soctherm_oc':
            soctherm_oc = hwmon

    soctherm_irqs = [SocthermIRQ(p) for p in soctherm_oc.glob('oc*irq_cnt')]

    while True:
        for irq in soctherm_irqs:
            if irq.get_new_event() > 0:
                return ('throttle-alert', 'Over-current')
        time.sleep(1)
